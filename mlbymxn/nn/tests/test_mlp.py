...
from mlbymxn.nn import MultiLayerPerceptron
from mlbymxn.utils import add_bias


class TestMultiLayerPerceptronBase(TestCase):

    @classmethod
    def setUpClass(cls):
        cls.X = add_bias(np.array([
        ...
        cls.Y = np.array([[0],[1],[0],[0]])
        cls.theta = np.concatenate((theta1.flatten(), theta2.flatten(), theta3.flatten()))


class TestMultiLayerPerceptron(TestMultiLayerPerceptronBase):

    def setUp(self):
        self.testee = MultiLayerPerceptron((4, 3), 1)

    def test_predict(self):
        got = self.testee.predict(self.theta, self.X)
        expected = [
            [0.55865298],
            [0.52006807],
            [0.51071853],
    ...
    def test_loss_function(self):
        got = self.testee.loss_function(self.theta, self.X, self.Y)
        assert_almost_equal(got, 0.740289031397, places=5)

    def test_gradient(self):
        got = self.testee.gradient(self.theta, self.X, self.Y)
        expected = [
...
class TestMultiLayerPerceptronNaiveImpl(TestMultiLayerPerceptronBase):

    def setUp(self):
        self.testee = MultiLayerPerceptron((4, 3), 1, use_naive_impl=True)
...
class TestMultiLayerPerceptronWithL2Reg(TestMultiLayerPerceptronBase):

    def setUp(self):
        self.testee = MultiLayerPerceptron((4, 3), 1, l2_reg=0.1)
